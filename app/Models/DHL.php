...
class DHL extends Model
{
            ...
            //foreach($result['consignments'] as $mainKey => $mainValue) {
                //if (count($mainValue['products']) > 0){
                    //foreach($mainValue['products'] as $key => $value){
                        ...
                        $final[$serviceCode]['serviceCompany'] = Config::get('constants.DHL');
                        $final[$serviceCode]['serviceCode'] =  'dhl'; //$value['productionCode'];
    ...
    public function storeShipmentBook($request){
        ...
        $dhl_book_shipment_success = true;
        try {
            //$result = $this->client->post($url, $params, $data);

            // fake data
            $result = array(
                "_DGFBookingID" => "2e5c37b954d c49a0b43a404bc1cf59b9",
                "status" => "Accepted",
                "statusURL" => "https://api-sandbox.dhl.com/dgff/transportation/shipment-booking/status?_DGFBookingID=2e5c37b954dc49a0b43a404bc1cf59b9"
            );

        } catch (Exception $e){
            $dhl_book_shipment_success = false;
        }
        
        if($dhl_book_shipment_success){
            $shipment = new Book;
            ...
            $shipment->companyOrderId = $result['_DGFBookingID'];
            $shipment->pickupTime = 'Unknown';
            $shipment->companyName = Config::get('constants.DHL');
